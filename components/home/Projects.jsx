        ...
        <section className='flex flex-col justify-center items-center w-full px-5 mb-14'>
            <div className='flex flex-col justify-center items-center gap-3 lg:w-[75vw] w-full'>
                <div className='flex justify-center items-center gap-4 self-start'>
                    <h1 className='text-3xl font-semibold self-start text-slate-300 flex gap-2 items-center'>Projects</h1>
                </div>
                <div className='grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 justify-center items-stretch gap-5'>
                    {displayProjexts.map((projext, index) => (
                        ...
                        <Link href={projext.link} target="blank" className='self-start'><p className='flex gap-1 items-center text-slate-200 hover:text-slate-300 transition ease-in-out font-semibold'>Visit<span className='-rotate-45 text-2xl'><IoArrowForwardOutline /></span></p></Link>
                    </div>
                    ))}
                </div>
                <Link href='/projects' className='px-3 py-2 font-semibold bg-slate-200 hover:bg-slate-300 text-[16px] transition ease-in-out rounded-lg mt-2'>More Projects</Link>
            </div>
        </section>
    )
}
