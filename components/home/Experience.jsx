          ...
          <div className="grid grid-cols-1 md:grid-cols-2 justify-start items-center gap-5 w-full p-4 rounded-lg card">
            {experience.map((exp , index)=>(
                <div key={index} className='flex flex-col justify-start items-center gap-2 p-4 rounded-md h-full exp'>
                    <h1 className="text-xl font-semibold   self-start mt-0">{exp.name}</h1>
                    <p className="text-[18px] font-medium   self-start my-auto">{exp.desc}</p>
                    <p className="text-[18px] font-semibold   self-start mb-0">{exp.date}({exp.role})</p>
                </div>
            ))}
          </div>
        </div>
